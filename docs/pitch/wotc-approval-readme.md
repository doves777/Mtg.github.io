# THE MILLION — Wizards of the Coast Approval Briefing

Pitch deck and research notes for **getting written authorization from Wizards of the Coast** before planning a $1,000,000 Magic championship.

**Open the deck:** [`pitch/wotc-approval/index.html`](../../pitch/wotc-approval/index.html)  
**Send path (public):** [tournaments@wizards.com](mailto:tournaments@wizards.com)  
**PDF:** open HTML → Print → Save as PDF

---

## Bottom line

Under Wizards’ public **Community Tournament Guidelines**, you may run a small community event **without a license** only if you stay inside hard caps — including a **$5,000 maximum** on all payments to participants and a **$25 maximum** entry fee. A **$1,000,000** prize event is far outside those caps and **requires prior written permission**. Do not announce, sell sponsorships, or open registration until that writing exists.

---

## Historically: what has been needed to run large Magic events?

Wizards has never published a single “apply here for a $1M open” form. Large events have instead fallen into a few **controlled lanes**:

### 1. Official Premier Play (Wizards-run or Wizards-selected)

Per current Magic Tournament Rules, **Premier** events are run by Wizards or select tournament organizers. Examples named in recent MTR language include:

- Magic World Championship  
- Pro Tour / Pro Tour Qualifier  
- Spotlight Series  
- Regional Championship / Regional Championship Qualifier  

These use Wizards systems (e.g. EventLink), invitation policy, and official branding. You do **not** self-declare a Premier Event.

### 2. Exclusive large-event partnership (GP / MagicFest era)

In **2017**, Wizards announced that from **2018** **ChannelFireball Events (CFBE)** would be the **exclusive global partner** to run Grand Prix — explicitly to improve consistency and quality of the largest public gatherings. Those weekends later branded as **MagicFest**.  

**Implication:** flagship paper events at GP scale were treated as a **partner appointment**, not an open franchise anyone could claim.

### 3. Independent large cash series (with careful non-affiliation)

Operators such as **StarCityGames** historically ran large **Open** series that:

- Paid substantial cash outside the official Pro Tour / GP purse structure  
- Used Magic rules culture (Competitive REL norms, judge staff, DCI/Wizards suspension respect)  
- Positioned themselves as **complementing** Organized Play, not as official Wizards championships  
- Avoided claiming Wizards sponsorship/endorsement without permission  

Some independent organizers later became **official partners** for pieces of the modern Premier structure (e.g. Regional Championship hosting). That is a negotiated relationship, not a default right.

### 4. Store-level WPN sanctioning (not the $1M path)

**WPN** brick-and-mortar stores can sanction FNM, Prereleases, Store Championships, and (by tier) RCQs through EventLink. That is the path for **local / regional** official play — **not** for inventing a branded million-dollar world championship.

### 5. Community / Arena guidelines → written license when you exceed them

Wizards’ **Community Tournament Guidelines** (primarily framed for community/Arena organizers) state that if your event **fits** the guidelines, you do **not** need a license; if it **doesn’t**, contact **tournaments@wizards.com**.

Triggers that push you into “need written permission” for something like THE MILLION include:

| Constraint (public guidelines) | THE MILLION |
| --- | --- |
| Max **$5,000** total payments to participants (prizes, fees, travel, etc.) | **$1,000,000** prize pool |
| Max **$25** entry fee | Likely higher for a championship weekend |
| No paywalled broadcast; TV/other media need prior permission | Global production ambitions |
| Cannot say the event is **sponsored by / official** Wizards without consent | Any official marks or co-branding |
| **Sponsors subject to Wizards approval**; prohibited categories (other game companies, alcohol, tobacco, drugs, weapons, adult content, etc.) | Corporate title sponsors must be cleared |
| No confusing use of Wizards trademarks / domains / social handles | Event naming + creative must be licensed or carefully non-infringing |

### 6. Brand / marketing controls (always)

Even with operational skill, **IP use** is gated:

- **Fan Content Policy** — no unauthorized logos/trademarks  
- **WPN Marketing Materials Policy** — WPN assets only for WPN members promoting products/sanctioned events; no false endorsement  
- Convention “special guest” requests exist; **unattended convention support** is currently described as not provided  

---

## What to put in a WotC approval packet (checklist)

Use the HTML deck as the leave-behind; attach or follow with:

1. **Explicit asks** — prize-cap waiver; model A/B/C (licensed partner / Premier collaboration / authorized independent); mark license; sponsor + broadcast approval rights for Wizards  
2. **Non-confusion plan** — naming, disclaimers, calendar vs Regional Championships / Spotlight Series  
3. **Integrity plan** — Head Judge, MTR/IPG, suspension handling, escrowed prizes  
4. **Brand-safety plan** — prohibited sponsor list adherence; Wizards veto  
5. **Legal entity** — who is the organizer, insurance, indemnification  
6. **No public sales until signed** — stated as a hard rule in the briefing  

---

## Deck contents (13 slides)

1. Cover — request for written authorization  
2. Why we stop before planning (guideline caps)  
3. Historical lanes (Premier / exclusive partner / independent)  
4. What approval typically covers  
5. Four-point ask  
6. Models A / B / C  
7. Event concept (pending approval)  
8. Brand protection  
9. Competitive integrity checklist  
10. Organized Play alignment  
11. Organizer readiness  
12. Proposed process  
13. Close + contacts  

Visual language differs from the **corporate sponsor** deck on purpose: this is an **approval / brand-protection** briefing, not a media-rights sales pitch.

---

## Sources (public)

- [Community Tournament Guidelines](https://company.wizards.com/en/community-tournament-guidelines)  
- [Fan Content Policy](https://company.wizards.com/en/legal/fancontentpolicy)  
- [WPN Marketing Materials Policy](https://wpn.wizards.com/en/marketing-materials-policy)  
- [Magic Tournament Rules (WPN media)](https://media.wizards.com/ContentResources/WPN/MTG_MTR_2025_Jul%2025_EN.pdf)  
- [Premier Tournament Invitation Policy](https://assets.ctfassets.net/ryplwhabvmmk/6YDZEeJXf70PuKG103ohdH/d5a55e0ac4aea87d0fcb21069f93b8a6/MTG_PTIP-2425_2024-09-27.pdf)  
- [Convention Support / Special Guest Requests](https://magic-support.wizards.com/hc/en-us/articles/360038749071-Convention-Support-and-Special-Guest-Requests)  
- Wizards announcement era coverage: CFBE exclusive GP partnership (2017) via Magic Organized Play / contemporary reporting  

Policies change. Treat this as a research brief for outreach, not legal advice — have counsel review before sending.
