...
### 2026-09-06 — At-rest vulnerability scan (automation)
...
- **Lesson:** August 2026 Next.js criticals (15.5.24) still do not yield a reportable chain here: Windows path-traversal RCE needs a Windows host (this scaffold is Linux/macOS); AVIF image-optimizer RCE needs `/_next/image` processing attacker-controlled AVIF, but this app uses raw `<img>` tags and has no `images.remotePatterns`. POS checkout remains localStorage-only. Skip GitHub comments/PRs for scan-only runs.
